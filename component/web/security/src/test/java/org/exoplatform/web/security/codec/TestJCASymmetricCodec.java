...
public class TestJCASymmetricCodec extends TestCase
{
   ...
   public void testDefaultCodec()
   {
      ...
      config.put("gatein.codec.jca.symmetric.transformation", "DES");
      config.put("gatein.codec.jca.symmetric.key.file", keyFile.getPath());
      config.put("gatein.codec.jca.symmetric.key.store.type", "JCEKS");
      config.put("gatein.codec.jca.symmetric.key.alias", "gateinCodec");
      config.put("gatein.codec.jca.symmetric.key.password", "gtn");
   ...
   public void testCodecWithGeneratedKey() throws Exception
   {
      ...
      char[] testPass = "testPass".toCharArray();
      ...
      if(!f.exists())
      {
         ...
         tmpStore.load(null, testPass);
         tmpStore.setEntry(alias, new KeyStore.SecretKeyEntry(tmpSecretKey), new KeyStore.PasswordProtection(testPass));
         OutputStream out = new FileOutputStream(f);
         try
         {
            tmpStore.store(out, testPass);
         }
         finally
         {
      ...
      Map<String, String> config = new HashMap<String, String>();
      config.put("gatein.codec.jca.symmetric.transformation", "DES");
      config.put("gatein.codec.jca.symmetric.key.file", f.getPath());
      config.put("gatein.codec.jca.symmetric.key.store.type", "JCEKS");
      config.put("gatein.codec.jca.symmetric.key.alias", alias);
      config.put("gatein.codec.jca.symmetric.key.password", "testPass");
